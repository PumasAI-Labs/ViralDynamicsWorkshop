...
_vpop = pop_pd[21:40] # Validation set: subjects 41 onward
...
nn_hiv = @model begin
    ...
    @param begin
        ...
        NN1 ∈ MLPDomain(5, 3, 3, (1, identity); reg = L2(1.0), act=softplus)
        NN2 ∈ MLPDomain(3, 2, 2, (1, identity); reg = L2(1.0), act=softplus)

        # NN1 ∈ MLPDomain(5, 7, 7, (1, identity); reg = L2(1.0))
        # NN2 ∈ MLPDomain(3, 5, 5, (1, identity); reg = L2(1.0))
    ...
    @vars begin
        ...
        INH = Conc / (Conc + IC50)
        β = R0 * d / λ / V
        infect_term = iNN_INF(T/100, V/100, (1 - INH))[1]
        prod_term   = iNN_VIR(I/100)[1]
    end
    @dynamics begin
...
serialize(joinpath(ARTIFACTS_DIR, "hiv_pkpd_nn.jls"), fit_pkpd)
