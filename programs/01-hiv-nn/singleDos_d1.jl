...
# obstimes
dense_hr8 = vcat(0, 0.02, 0.08, 0.2, 0.3, 0.5)   
dense_day = vcat(1, 2, 3, 7, 14, 21, 28)   
# day42 = vcat(7:7:43)

obstimes = unique(sort(vcat(dense_hr8,dense_day)))


ev4 = DosageRegimen(900; time = 0, cmt = 1, evid=1, ii=0, addl=0)
pop_ev4 = map(i -> Subject(id = i, events = ev4), 1:30)
...
df_pop_ev4[!, :Dose] .= "900 mg single dose"
...
CSV.write("single_dose.csv", pkpddata_sim)
