...
upDated <-  simData %>% 
  select(id,time,Concentration,Virus,DV,evid,amt,cmt,rate,route,Dose) %>% 
  
  mutate(Concentration = ifelse(Concentration <= 0, NA, Concentration)) %>% 
  
  mutate(Virus = ifelse(time %in% viralData, NA, DV)) %>%
  
  select(id,time,Concentration,Virus,evid,amt,cmt,Dose)
  
  # mutate(FLG = ifelse(iDV > 0 | amt>0, 2, 1)) %>% 
  # mutate(FLG = ifelse(is.na(FLG), 1, FLG))


write.csv(file="hiv-pkpd-data.csv", upDated, row.names = F,
na = "")
